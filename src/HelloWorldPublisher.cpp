        ...
        const std::vector<std::string>& whitelist)
{
    ...
    std::string readStr((std::istreambuf_iterator<char>(readfile)),  std::istreambuf_iterator<char>());
    hello_.message(readStr);
    ...
    if (samples == 0)
    {
        while (!stop_)
        {
            if (publish(false))
            {
                ...
                printf("[RTCP] Message length:%ld and with index:%lf SENT\n",hello_.message().size(),hello_.index());
            }
